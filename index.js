...
client.on("message", async message => {
    ...
    if (command === "countdown") {
        ...
        games_channel.send("Countdown: " + seconds + "s").then(message => {
    ...
    if (command === "squadvote" || command === "sqv" && message.channel.id === config.host_channel_id) {
        const host_channel = client.channels.get(config.host_channel_id);
        time = config.default_timer
        var reactionArray = [];
        var emojiList = ['1⃣', '2⃣', '3⃣', '4⃣', '5⃣', '6⃣', '7⃣', '8⃣', '9⃣', '🔟'];

        if (!args.length) {
            message.channel.send(`You didn't provide squad sizes, ${message.author}! Using the defaults...`)
            squad_vote_message = "Vote for the squad size for the next game!",
                host_channel.send(squad_vote_message)
                    .then(async function (message) {
                        reactionArray[0] = await message.react(emojiCharacters[1])
                        reactionArray[1] = await message.react(emojiCharacters[2])
                        reactionArray[2] = await message.react(emojiCharacters[4])
                        reactionArray[3] = await message.react(emojiCharacters[8])
                    })
        } else {
            squad_vote_message = "Vote for the squad size for the next game!",
                host_channel.send(squad_vote_message)
                    .then(async function (message) {
                        reactionArray[0] = await (message.react(emojiCharacters[args[0]]))
                        reactionArray[1] = await message.react(emojiCharacters[args[1]])
                        reactionArray[2] = await message.react(emojiCharacters[args[2]])
                        reactionArray[3] = await message.react(emojiCharacters[args[3]])
                        reactionArray[4] = await message.react(emojiCharacters[args[4]])
                        reactionArray[5] = await message.react(emojiCharacters[args[5]])
                        reactionArray[6] = await message.react(emojiCharacters[args[6]])
                        reactionArray[7] = await message.react(emojiCharacters[args[7]])
                        reactionArray[8] = await message.react(emojiCharacters[args[8]])
                        reactionArray[9] = await message.react(emojiCharacters[args[9]])
                    }).catch(console.error);
        }
        if (time) {
            host_channel.send(`The vote has started and will last ${time} minute(s)`)
        }
        if (time) {
            setTimeout(() => {
