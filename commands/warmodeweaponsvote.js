...
module.exports = async (message, args) => {
    ...
    const warmodewepChoices = [
        `${emojiCharacters[1]} for Default Weapons (Everything customizable)`,
    ...
    if (timer == 0) {
        games_channel.send(new Discord.MessageEmbed()
            .setColor(0x3366ff)
            .setTitle('Random War Mode weapon selection')
            .setDescription('The War Mode weapon kit for the next game will be randomly selected')
            .addField('Choices', warmodewepChoices.join('\n'), true)
            .addField('Selection', warmodewepChoices[Math.floor(Math.random() * Math.floor(warmodewepChoices.length))], true)
            .setTimestamp()
            .setFooter('', client.user.displayAvatarURL())
        ).catch(console.error);
        ...
        try {
            await games_channel.send(new Discord.MessageEmbed()
                .setColor(0x3366ff)
                .setTitle('Vote for War Mode weapons!')
                .setDescription('Please vote on the weapons for the next War Mode games!')
                .addField('Choose a reaction', warmodewepChoices.join('\n'))
                ...
                collector.on('end', reactions => {
                    ...
                    if (draws.length > 1)
                        fields = [
                            ...
                            {
                                name: 'The winning weapon choice is:',
                                value: `${winReact}`,
                                inline: true
                            }
                        ];
